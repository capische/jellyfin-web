...
const networkIdle = async ({ quiet = 750, timeout = 20000 } = {}) => {
    const deadline = Date.now() + timeout;
    const busy = () => [...inflight].some(request => !heldRequests.has(request));
    while (Date.now() < deadline && (busy() || Date.now() - lastNetworkActivity < quiet)) await sleep(100);
...
try {
    ...
    for (const [layout, width, height] of [['desktop', 1440, 900], ['mobile', 390, 844], ['tv', 1920, 1080], ['tv', 1280, 720]]) {
        ...
        if (expectAdmin) {
            ...
            if (!keptOk(keptState)) {
                throw new Error('Keep by ' + step + ' and Enter did not keep focus and state in ' + layout + ': ' + JSON.stringify(keptState));
            }
    ...
    await page.locator('.jfmod-discovery select').first().waitFor({ state: 'attached', timeout: 20000 }).catch(ignore);
    await selectDiscoveryLibraries();
    const firstAdd = page.locator('[data-jfmod-add]:not(:disabled)').first();
    await firstAdd.waitFor({ timeout: 20000 }).catch(ignore);
    const identity = await firstAdd.getAttribute('data-jfmod-add', { timeout: 1000 }).catch(() => null);
    ...
    const canonicalCount = await poll(() => page.locator(`[data-jfmod-tmdb-id="${successfulAdd.tmdbId}"]`).count(), count => count === 1);
    ...
    const firstPage = await poll(readFirstPage, result => result.targetLibraryId && result.ids.length >= 10);
    ...
    await enterFocused();
    await poll(() => movieIds().then(ids => ids.length), count => count > beforeMore, { timeout: 30000 });
    await networkIdle();
    ...
    const seedPage = await poll(async () => ({
    ...
    const continuation = await poll(() => page.evaluate(() => ({
        ids: Array.from(document.querySelectorAll('[data-jfmod-add^="movie:"]')).map(button => Number(button.dataset.jfmodAdd.split(':')[1])),
