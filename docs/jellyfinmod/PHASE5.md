...
evidence, 2026-09-18). Where it and the review amendments to PHASE4 in this checkout disagree,
this document records the disagreement under *Open questions* rather than choosing.
...
```json
{
  ...
  "client": { "id": "7777…", "name": "qBittorrent (isolated)", "openUrl": "https://…" },
  "seeding": null
}
```
...
- **Client semantics.** From the chosen client's actual API: the file-list fields (name, size,
  ...
  paused by the client's own share limit is distinguishable from one paused by hand. If the
  client is qBittorrent, extend the Phase 4 A5 driver; if Transmission, extend
  `TransmissionSeedClient` rather than adding a second RPC client.
...
- Effective goal = strictest of the grab's indexer snapshot, the client's observed per-torrent
  ...
  the goal's own semantics (Transmission: ratio or idle; qBittorrent: ratio or seeding time),
...
1. **Single mount on the Pi.** Will the isolated compose (and later production) expose downloads
   and the library to the Jellyfin container through one mount so hardlinks work? This is PLAN
   open question 3 restated with the bind-mount detail; it is a deployment decision. Consumed by
   gate 3 and I1.
...
9. **Newest Phase 4 contract vs. review amendments.** The `jellyfinmod-phase4` A1 evidence
   records qBittorrent 5.2.3 as the provisioned client and proceeds as if the client decision
   were settled, keeps one-Enter grab with correction in the client WebUI, stores the client
   credential under the isolated host directory rather than `plugin/.env`, and does not record a
   merged `origin/master` SHA. The review amendments keep the client (open question 6), the
   mistaken-grab rule (open question 7), the credential location (A9) and X1 open. Phase 5 needs
   the client and credential answers before I1; the mistaken-grab answer matters less once the
   queue exists, because UX §9's premise ("the queue is where a mistake gets undone") is
   satisfied by `DELETE /Queue/{id}`.
